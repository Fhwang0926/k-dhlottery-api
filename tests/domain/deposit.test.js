...
const deposit_1 = require("../../src/domain/deposit");
describe("Deposit", () => {
    it("should throw on non-digit amount", () => {
        expect(() => new deposit_1.Deposit("a")).toThrow("숫자를 입력하세요 (입력된 값: a).");
    });
    it("should throw on not allowed amount", () => {
        expect(() => new deposit_1.Deposit(777)).toThrow("입금 가능한 금액은 5천원, 1만원, 2만원, 3만원, 5만원, 10만원, 20만원, 30만원, 50만원, 70만원, 100만원입니다 (입력된 값: 777).");
    });
